...
if (0) {
  ...
  # "has-supports-predicate-representation" "LPG-td" "yes"
  my @entries3;
  foreach my $entry (@entries2) {
    my $size = scalar @$entry;
    if ($size == $maxkey) {
  ...
  my $tableofcontents = shift @entries3;
  my $title = shift @{$tableofcontents};

  my $entryid = 0;
  foreach my $entry (@entries3) {
    my $size = scalar @$entry;
    print "SIZE: $size\n";
    print "MAXKEY: $maxkey\n";
    if ($size == $maxkey) {
      my $j = 0;
      my $start = shift @$entry;
      print "START: $start\n";
      foreach my $item (@$entry) {
	if ($tableofcontents->[$j] ne "") {
	  my $formula = [Clean($title), Clean($tableofcontents->[$j]), Clean($start), Clean($item)];
	  push @formulae, $formula;
	}
	++$j;
      }
